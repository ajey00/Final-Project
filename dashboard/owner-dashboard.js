...
function renderOrders() {
    ...
    if (orders.length === 0) {
        container.innerHTML = '<div class="empty-state"><h3>No orders yet</h3><p>Orders will appear here in real time</p></div>';
        return;
    }
...
/* ─── TABLES ─────────────────────────────────────────── */
/**
 * Table status is derived entirely from live orders.
 * occupied  = any pending/confirmed order
 * delivered = delivered but unpaid orders (no pending/confirmed)
 * free      = no unpaid orders (all paid or none)
 */
function renderTables() {
    ...
    const tableStatus = {};
    for (let i = 1; i <= 10; i++) tableStatus[i] = 'free';

    orders.forEach(order => {
        const t = parseInt(extractTableNumber(order.tableNumber), 10);
        ...
        if (status === 'delivered') {
            if (tableStatus[t] !== 'occupied') tableStatus[t] = 'delivered';
            return;
        }
    ...
    container.innerHTML = Object.entries(tableStatus).map(([num, status]) => `
